const resolveApiBase = () => {
  if (window.VID2TUNE_API_BASE) return window.VID2TUNE_API_BASE;
  if (window.__VID2TUNE_API__) return window.__VID2TUNE_API__;

  const { protocol, hostname, port } = window.location;

  // When the frontend is served directly from the backend (same origin)
  if (port === "4000") {
  ...
  // Fallback for file:// or custom schemes.
  ...
  try {
    ...
    const message =
      error.name === "TypeError"
        ? `Tidak bisa menghubungi server backend di ${API_BASE}. Pastikan backend sedang berjalan dan dapat diakses.`
        : error.message;
    updateStatus(videoStatus, message, "error");
  } finally {
  ...
  try {
    ...
    const message =
      error.name === "TypeError"
        ? `Tidak bisa menghubungi server backend di ${API_BASE}. Pastikan backend sedang berjalan dan dapat diakses.`
        : error.message;
    updateStatus(audioStatus, message, "error");
  } finally {
